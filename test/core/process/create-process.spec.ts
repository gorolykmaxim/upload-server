...
import {CreateProcess, CreateProcessArgs} from "../../../backend/core/process/create-process";
...
describe('CreateProcess', function () {
    const args: CreateProcessArgs = new CreateProcessArgs('yes');
    ...
    beforeEach(function () {
        ...
        command = new CreateProcess((command1, args_, options) => {
            if (command1 === args.command) {
                return childProcess;
            }
        }, pidToProcess);
    });
    it('should start a process for the specified command and return PID', async function () {
        // when
        const actualPID: number = await executeAndReturnOutput(command, args).toPromise();
        // then
    ...
    it('should close STDIN of the create process on an error from the commands input', async function () {
        // when
        await executeAndReturnOutput(command, args, throwError(new Error())).toPromise();
        // then
